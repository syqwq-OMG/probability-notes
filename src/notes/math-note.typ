...
#let note(
  ...
  show figure.where(kind: "pic"): set figure(numbering: it => numbering(
    "1.1",
    counter(heading).get().first(),
    counter(figure.where(kind: "pic")).get().first(),
  ))
  ...
  show heading.where(level: 1): t => context {
    counter(figure.where(kind: "pic")).update(0)
